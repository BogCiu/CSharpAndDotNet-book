        ...
        static void Main(string[] args)
        {
            ...
            foreach (var friendName in friendNames)
            {
                switch (friendName)
                {
                    case string t when t.StartsWith("T"):
                        Console.WriteLine($"This friends name starts wth a 'T': {friendName} and is {t.Length - 1} letters long.");
                        break;
                    ...
                    case null:
                        ...
                        break;
                    case var x:
                        Console.WriteLine($"This is the var pattern of type: {x.GetType().Name}");
                        break;
                    default:
                        break;
                }
            ...
            foreach (var integer in myIntArray)
            {
                switch (integer)
                {
                    ...
                    case int value:
                        sum += value;
                        Console.WriteLine($"Integer number '{counter}' has a default value of {value}");
                        counter++;
                        break;
                    case null:
